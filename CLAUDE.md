...
1. **Data Management**
   (`src/cluster_analysis/data_management/task_data_management_template.py`)

   - Loads raw CSV data, cleans it, saves as pickle to `bld/data/`

1. **Analysis** (`src/cluster_analysis/analysis/task_analysis_template.py`)

   - Fits logit models and generates predictions
   - Iterates over `TEMPLATE_GROUPS` defined in `config.py`

1. **Final Outputs** (`src/cluster_analysis/final/task_final_template.py`)
...
- `src/cluster_analysis/config.py`: Central path definitions (`SRC`, `ROOT`, `BLD`,
...
- `src/cluster_analysis/`: Source code (hand-written)
